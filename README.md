...
- **PMTUD_Status**:
  ...
  - `Blackhole` — the packet was silently dropped, no ICMP reply (PMTUD blackhole / a firewall filters ICMP type 3), CONFIRMED by a `ping` summary line that explicitly reports 100% packet loss. Since 100% packet loss looks identical to transient loss from local overload, the DF test is retried once before `Blackhole` is reported as final. If `ping`'s "packet loss" line can't be parsed at all (unexpected wording, unusual output), that is deliberately NOT treated as a Blackhole — see `Error` below.
  ...
  - `Error` — either the ping test for this host failed with an unexpected exception (e.g. `OSError: Too many open files` from `--workers` set too high), or `ping`'s output couldn't be classified at all (no recognizable "packet loss" line and no "rtt" summary line — an unrecognized `ping` output format, not a confirmed failure of any kind). The rest of the scan continues; the reason is recorded in the note field. (A "packet loss" line that's missing but where an `rtt` summary line IS present is instead reported as `OK`, not `Error` — a real rtt value is solid proof a full-size reply arrived, so it's trusted over the missing loss line.)
