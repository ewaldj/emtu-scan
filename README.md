...
- **Full Host-by-Host Mode** — `--full-scan` skips subnet elimination entirely when partial-alive subnets need to be caught completely. Hosts are ICMP-checked in memory-bounded batches (see `--allow-huge-scan` below) rather than building one giant target list for the whole range up front — confirmed on real hardware to prevent the OOM-kill that a large `--full-scan` used to hit on both 4GB and 16GB machines
...
- **Excel + Text Output** — results written as a pre-sorted `.xlsx` (3 tabs, AutoFilter) and matching plain-text files, no `openpyxl` needed. Each worksheet is streamed directly into the `.xlsx` in bounded row batches with ZIP64 support rather than built as one giant string in memory first — confirmed on real hardware to prevent both an OOM-kill and a separate "File size unexpectedly exceeded ZIP64 limit" failure that a `--full-scan` of a `/8` (~16.7M hosts) used to hit at this step, well after the scan itself had already completed successfully
...
Measured on real hardware (Linux, default options unless noted: `--count 3 --timeout 1000 --workers 50 --icmp-timeout 1.0 --icmp-retries 2`). Actual runtime depends heavily on how many hosts are actually alive (each one needs the full MTU/DF `ping` test, the slow part) and on network/router responsiveness, so treat these as ballpark figures, not a guarantee:

- `10.0.0.0/8 --full-scan` (16,777,214 hosts, no `--mask` elimination — every host individually ICMP-checked): **~34 minutes** (2054.8s). Of those, 16,977 hosts (0.1%) were reachable and went through the full MTU/DF test.
- A `-f <file> --full-scan` recheck of 581 specific hosts (e.g. the `recheck-blackhole.txt` this scan itself produced): **~13 seconds**.

The huge gap between these isn't just host count — a `--full-scan` of a whole `/8` ICMP-checks all 16.7M addresses with no elimination, while a small recheck file only has real hosts to begin with. Adding `--mask` (so dead subnets get eliminated by a first/last-host probe before being expanded) instead of `--full-scan` is usually far faster than a full range scan when most of the range is expected to be empty - `--full-scan` is worth its slower per-address cost specifically when partial-alive subnets need to be caught completely, or you're intentionally re-testing a known list of hosts (as with the recheck files above).
...
| `--allow-huge-scan` | required to proceed when the given network(s) total more than ~20,000,000 addresses (roughly a /8 or larger) **without** `--mask` splitting, or with `--full-scan`. The ICMP reachability check itself is processed in memory-bounded batches (60,000 hosts in flight at a time, regardless of total scan size — this is what used to OOM-kill the process on `--full-scan` runs before it was fixed, and matching the batch size to the ICMP layer's own internal chunk size is also why `[scan]` output for a mostly-dead range now updates every few seconds instead of visibly pausing for tens of seconds at a time), but every host's result is still kept in memory until the run finishes and the final `.xlsx`/`.txt` reports are written, so a very large range still needs proportionally more memory and a long runtime overall. Splitting into smaller networks, or adding `--mask` (so subnet elimination skips whole dead ranges instead of testing every host), is usually the better fix for genuinely huge ranges |
...
- **PMTUD_Status**:
  ...
  - `Blackhole` — the packet was silently dropped, no ICMP reply (PMTUD blackhole / a firewall filters ICMP type 3), CONFIRMED by a `ping` summary line that explicitly reports 100% packet loss AND no `rtt min/avg/max` line is present anywhere in that same output. Since 100% packet loss looks identical to transient loss from local overload, the DF test is retried once before `Blackhole` is reported as final — but that one retry, milliseconds later, doesn't reliably escape a busy scan's own resource contention or router-side ICMP rate-limiting (many routers throttle how many Fragmentation-Needed replies they generate per second, so which of many simultaneously-tested hosts behind the same router still gets a real reply varies run to run). A `Blackhole` (or `DF-Needed`) result from a large, heavily-parallel scan is therefore worth confirming with a separate, low-concurrency recheck rather than treated as final on its own — see `recheck-blackhole.txt` / `recheck-df.txt` under Output below, written automatically for exactly this. If a real `rtt` line IS present in the same output — even alongside a "packet loss" line missing entirely or explicitly claiming 100% (seen on real hardware during busy scans, where local overload can make `ping`'s own loss/rtt bookkeeping momentarily self-contradictory) — that's trusted over the loss-line claim and reported as `OK` instead, not silently guessed as a false Blackhole.
...
- `recheck-blackhole.txt`, `recheck-df.txt` — every host that ended up `Blackhole` / `DF-Needed` this run, one bare `/32` per line, ready to feed straight back in as `-f recheck-blackhole.txt --full-scan` (or `recheck-df.txt`). Always written, even empty. A large, heavily-loaded scan (e.g. a `--full-scan` of a `/8` with many workers) makes both statuses noticeably less reliable per host — confirmed on real hardware: rechecking the exact same host list in isolation, with no other scan competing for local CPU/socket/bandwidth, repeatedly gave different specific hosts DF-Needed vs. Blackhole between runs, most likely from ICMP rate-limiting on routers along the path (many devices throttle how many Fragmentation-Needed replies they generate per second) combined with local contention during the big scan itself. Rechecking with these files — smaller, low-concurrency, run more than once if in doubt — gives a materially more trustworthy answer for exactly the hosts worth a second look.
